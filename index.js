...
client.once(Events.ClientReady, async c => {
  ...
  try { await realmBridge.init(c); } catch (e) { console.error('Realm bridge init:', e); }
});
  ...
  try {
    ...
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'ticket_type') {
        ...
        const allowed = ['ticket_support','ticket_report','ticket_bug','ticket_appeal','ticket_partner'];
        if (!allowed.includes(value)) return interaction.reply({ content: '❌ Invalid ticket type.', flags: MessageFlags.Ephemeral });
        return createTicket(interaction, value.replace('ticket_', ''));
      }
      return;
    }
