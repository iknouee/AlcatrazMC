...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('status-channel').setDescription('Set the live Realm status channel').addChannelOption(o => o.setName('channel').setDescription('Realm status channel').setRequired(true))),
  async execute(interaction) {
    ...
    if (sub === 'status-channel') patch.statusChannelId = interaction.options.getChannel('channel').id;
